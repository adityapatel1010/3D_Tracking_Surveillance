...
app = FastAPI(title="SACRT Ticket Evasion Detection")
...
# Store reference objects per video
reference_objects: Dict[str, dict] = {}  # filename -> {object, first_frame}

@app.on_event("startup")
async def startup_event():
    """Initialize models when server starts"""
    global tracker, sam_selector, depth_estimator
    try:
        ...
        logger.info("✅ RT-DETR and SmolVLM loaded successfully")
        
        # Load SAM for reference object selection
        logger.info("🔄 Loading SAM model...")
        from reference_object_selector import ReferenceObjectSelector
        sam_selector = ReferenceObjectSelector(use_mobile=True)
...
@app.get("/api/first-frame/{filename}")
async def get_first_frame(filename: str):
    """Extract and return the first frame of uploaded video as base64 image"""
    try:
        ...
        logger.info(f"Extracting first frame from: {filename}")
        ...
        logger.info(f"First frame extracted: {width}x{height}, {total_frames} frames @ {fps} fps")
        ...
        logger.error(f"Failed to extract first frame: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Frame extraction failed: {str(e)}")

@app.post("/api/segment-object")
async def segment_object(request: dict):
    """Segment object at clicked coordinates using SAM"""
    try:
        if sam_selector is None:
        ...
        if not filename or x is None or y is None:
            raise HTTPException(status_code=400, detail="Missing filename or coordinates")
        
        if filename not in reference_objects or 'first_frame' not in reference_objects[filename]:
            raise HTTPException(status_code=404, detail="First frame not found. Please extract first frame first.")
        
        logger.info(f"Segmenting object at ({x}, {y}) in {filename}")
        
        frame = reference_objects[filename]['first_frame']
        
        # Set image in SAM
        sam_selector.set_image(frame)
        
        # Segment object
        ref_object = sam_selector.segment_from_point(int(x), int(y))
        
        # Create visualization with mask overlay
        from reference_object_selector import ReferenceObjectSelector
        mask_overlay = ReferenceObjectSelector.visualize_mask(
            frame, ref_object.mask, alpha=0.4, color=(0, 255, 0)
        )
        
        # Draw contour
        mask_overlay = ReferenceObjectSelector.draw_mask_contour(
            mask_overlay, ref_object.mask, color=(0, 255, 0), thickness=3
        )
        
        # Encode mask overlay
        _, buffer = cv2.imencode('.jpg', mask_overlay, [cv2.IMWRITE_JPEG_QUALITY, 95])
        overlay_base64 = base64.b64encode(buffer).decode('utf-8')
        
        # Encode binary mask as PNG
        mask_rgb = np.zeros((ref_object.mask.shape[0], ref_object.mask.shape[1], 3), dtype=np.uint8)
        mask_rgb[ref_object.mask > 0] = [0, 255, 0]
        _, mask_buffer = cv2.imencode('.png', mask_rgb)
        mask_base64 = base64.b64encode(mask_buffer).decode('utf-8')
        
        # Store reference object temporarily
        reference_objects[filename]['temp_ref_object'] = ref_object
        
        logger.info(f"   Object segmented: area={ref_object.area}, bbox={ref_object.bbox}")
        ...
        return JSONResponse({
            ...
            "centroid": ref_object.centroid,
            "area": ref_object.area
        })
...
@app.post("/api/confirm-reference")
async def confirm_reference(request: dict):
    """Confirm and store the reference object selection"""
    try:
        filename = request.get('filename')
        ...
        if filename not in reference_objects or 'temp_ref_object' not in reference_objects[filename]:
            raise HTTPException(status_code=404, detail="No segmented object found")
        
        # Move temp object to confirmed
        reference_objects[filename]['ref_object'] = reference_objects[filename].pop('temp_ref_object')
        
        ref_obj = reference_objects[filename]['ref_object']
        
        logger.info(f"✅ Reference object confirmed for {filename}")
        logger.info(f"   Centroid: {ref_obj.centroid}, Area: {ref_obj.area}")
        
        return JSONResponse({
            "status": "success",
            "message": "Reference object confirmed",
            "centroid": ref_obj.centroid,
            "bbox": ref_obj.bbox
        })
        
    except Exception as e:
        logger.error(f"Failed to confirm reference: {str(e)}", exc_info=True)
...
async def process_video(
    ...
    try:
        ...
        if tracker is None or depth_estimator is None:
            logger.error("Models not loaded")
            raise HTTPException(status_code=503, detail="Models are still loading. Please wait and try again.")
        
        # Check if reference object is selected
        if filename not in reference_objects or 'ref_object' not in reference_objects[filename]:
            raise HTTPException(status_code=400, detail="Reference object not selected. Please select reference object first.")
        ...
        ref_object = reference_objects[filename]['ref_object']
        
        logger.info(f"Starting processing for: {filename}")
        logger.info(f"   Reference object at: {ref_object.centroid}")
        processing_status.update("processing", 0, "Initializing reference-based tracker...")
        processing_status.current_file = filename
        
        await broadcast_status({
            "status": "processing",
            "progress": 5,
            "message": "Preparing reference-based tracker..."
        })
        
        # Import reference-based detector
        from reference_tap_detector import ReferenceBasedTapDetector, visualize_reference_tracking
        ...
        await broadcast_status({
            "status": "processing",
            "progress": 10,
            "message": "Starting distance-based tracking..."
        })
        ...
        logger.info("Creating visualization video with reference object...")
        await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: visualize_reference_tracking(
                video_path=str(video_path),
        ...
        experiment_video_path = experiment_folder / f"processed_video{output_path.suffix}"
        ...
        summary_path = experiment_folder / "experiment_summary.txt"
        with open(summary_path, 'w') as f:
            f.write("="*80 + "\n")
            f.write("REFERENCE-BASED TRACKING EXPERIMENT SUMMARY\n")
            f.write("="*80 + "\n\n")
            f.write(f"Experiment Folder: {experiment_folder.name}\n")
            f.write(f"Input Video: {filename}\n")
            f.write(f"Processing Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write("REFERENCE OBJECT:\n")
            f.write(f"  - Centroid: {ref_object.centroid}\n")
            f.write(f"  - Bounding Box: {ref_object.bbox}\n")
        ...
        logger.info(f"All experiment files organized in: {experiment_folder}")
        ...
        shutil.copy(str(results_path), str(experiment_folder / "results.json"))
        ...
        return JSONResponse({
            ...
            "output_video": output_filename,
            "results_file": results_filename,
            "experiment_folder": str(experiment_folder.name),
            "results": results,
            "message": "Video processed successfully with reference-based tracking"
        })
